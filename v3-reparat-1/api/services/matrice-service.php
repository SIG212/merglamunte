...
/**
 * Generează mesaj personalizat pentru decizie
 */
if (!function_exists('genereazaMesajDecizie')) {
function genereazaMesajDecizie($status, $context, $meteo_status, $dificultate) {
    $masiv = $context['masiv_display'];
    $altitudine = $context['altitudine_tinta'];
    $dificultate = $context['dificultate'];
    $sezon = $context['sezon'];
    $zona = $context['zona'];
}
    // Mesaje pentru ROȘU
    if ($status_final === 'ROSU') {
        ...
        if ($nivel === 'incepator') {
            ...
            if ($sezon === 'iarna' && $dificultate >= 3) {
                return "🔴 Traseele de iarnă la această altitudine necesită experiență avansată în drumețiile de iarnă și echipament tehnic complet (colțari, cască, piolet). Recomandăm trasee mai simple sau o drumeție vara.";
            }
        }
...
if (!function_exists('aplicaMatrice')) {
    /**
     * Alias pentru aplicareMatriceRisc (pentru compatibilitate)
     */
    function aplicaMatrice($nivel_experienta, $context, $no_go_count, $caution_count) {
        // Determinăm meteo_status din counters
        if ($no_go_count >= 1) {
            $meteo_status = 'ROSU';
